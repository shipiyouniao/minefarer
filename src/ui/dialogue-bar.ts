...
export function dialogueBar(
  ...
  return `<dialog class="dialogue-bar ${kind}-dialogue" aria-labelledby="${speaker}"><div class="dialogue-cast"><span ${story ? 'data-story-speaker="player"' : 'data-signal-listener'}>${player}</span><span ${story ? 'data-story-speaker="lumi"' : 'data-signal-portrait'}>${partner}</span></div><div class="dialogue-copy"><strong id="${speaker}" ${story ? 'data-story-speaker-name' : ''}></strong><p role="status" ${story ? 'data-story-dialogue-line' : 'data-signal-line'}></p><button type="button" class="dialogue-next" ${story ? 'data-story-action="dialogue"' : 'data-signal-next'}>${message(language, 'story.dialogue-next')} →</button></div></dialog>`
}
