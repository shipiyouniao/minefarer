...
export function storyMap(state: StoryViewState): string {
  ...
  return `<div class="story-map" data-map-level="${level}" data-map-scene="${scene}" data-map-region="${region}"><div class="atlas-toolbar">${navigation}</div><div class="atlas-heading"><h3>${escapeHtml(title)}</h3>${campCard}</div><div class="atlas-canvas"><div class="atlas-viewport" tabindex="0" role="group" aria-label="${escapeHtml(title)}" data-enter-label="${message(lang, 'story.atlas-enter')}"><div class="atlas-scene">${map.drawing}</div></div></div>${zoom}</div>`
}
